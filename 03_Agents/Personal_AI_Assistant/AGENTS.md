...
8. Wanderlog 行程：當 Melessa 要求在 Wanderlog **加景點、改 note、同步清單到某個 list／section**，或提供 `connect.sid` 要改 Wanderlog 時，**必須先讀並嚴格遵守** `02_Knowledge_Base/skills/wanderlog-places/SKILL.md`（等同 `~/.cursor/skills/wanderlog-places/SKILL.md`）；用 `wanderlog-mcp` API；未提供 `connect.sid` 時先問並教 DevTools 取得方式
