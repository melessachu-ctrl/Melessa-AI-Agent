...
8. Wanderlog 行程：當 Melessa 要求在 Wanderlog **加景點、改 note、同步清單到某個 list／section**，或提供 `connect.sid` 要改 Wanderlog 時，**必須先讀並嚴格遵守** `02_Knowledge_Base/skills/update-wanderlog/SKILL.md`（等同 `~/.cursor/skills/update-wanderlog/SKILL.md`）；note 標題預設 **`地區｜景點名`**；用 `wanderlog-mcp` API；**先查環境變數 `WANDERLOG_COOKIE`**（Cloud Agent Secret），有則直接用、不要再問；沒有才問並教 DevTools。勿把個人帳密寫進 chat／repo
